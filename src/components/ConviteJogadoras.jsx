...
function ConviteJogadoras({ clubeId, jogadorasConvidadas, onJogadorasAtualizadas }) {
  ...
  const carregarJogadorasDisponiveis = async () => {
    try {
      ...
      const jogadorasFiltradas = todasJogadoras.filter(jogadora => 
        !jogadorasConvidadas.some(convidadas => convidadas.id === jogadora.id)
      );
  ...
  const handleConvidarJogadora = async (jogadora) => {
    try {
      ...
      await jogadoraServiceRealtime.convidarJogadoraParaClube(jogadora.id, clubeId);
  ...
  const jogadorasFiltradas = jogadorasDisponiveis.filter(jogadora =>
  ...
  return (
    <div className="space-y-6">
      <div className="bg-gray-50 rounded-lg p-4">
        <h3 className="text-lg font-semibold text-[#521E2B] mb-2">Status do Time</h3>
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm text-gray-600">
              Jogadoras convidadas: <span className="font-semibold">{jogadorasConvidadas.length}/5</span>
            </p>
            <p className="text-sm text-gray-600">
              Restam: <span className="font-semibold">{jogadorasRestantes}</span> jogadoras
            </p>
          </div>
      ...
      {jogadorasConvidadas.length > 0 && (
        <div>
          <h3 className="text-lg font-semibold text-[#521E2B] mb-3">Jogadoras Convidadas</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {jogadorasConvidadas.map(jogadora => (
              <div key={jogadora.id} className="bg-green-50 border border-green-200 rounded-lg p-3">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="font-semibold text-green-800">{jogadora.nome}</p>
      ...
      <div>
        <h3 className="text-lg font-semibold text-[#521E2B] mb-3">Convidar Jogadoras</h3>
        <div className="mb-4">
          <input
            type="text"
        ...
        {isLoading ? (
          ...
          <div className="text-center py-8 text-gray-500">
            {busca ? 'Nenhuma jogadora encontrada com esse filtro' : 'Nenhuma jogadora disponível'}
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 max-h-96 overflow-y-auto">
            {jogadorasFiltradas.map(jogadora => (
              <div key={jogadora.id} className="bg-white border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                <div className="flex items-center justify-between">
                  <div className="flex-1">
                    <p className="font-semibold text-gray-800">{jogadora.nome}</p>
                    <p className="text-sm text-gray-600">{jogadora.posicao}</p>
                    <p className="text-xs text-gray-500">{jogadora.cidade}, {jogadora.estado}</p>
                  </div>
                  <button
        ...
        <div className="flex items-start space-x-3">
          ...
          <div>
            ...
            <p className="text-sm text-blue-700 mt-1">
              Convide jogadoras para formar seu time. Você precisa de pelo menos 5 jogadoras para poder desafiar outros times.
            </p>
          </div>
        </div>
      </div>
